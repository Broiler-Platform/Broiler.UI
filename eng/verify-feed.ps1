...
if ($Target -eq 'github') {
    ...
    $githubSource = "<add key=`"broiler-github`" value=`"https://nuget.pkg.github.com/$owner/index.json`" />"
    $githubMapping = '<packageSource key="broiler-github"><package pattern="Broiler.*" /></packageSource>'
}
