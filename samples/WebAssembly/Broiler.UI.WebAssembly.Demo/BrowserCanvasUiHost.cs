...
/// <summary>
/// Queued dispatcher that coalesces managed work onto the browser animation-frame tick. Posting
/// a callback schedules a frame; the frame drains the queue before rendering.
/// </summary>
...
/// <summary>
/// The gallery's <see cref="IUiHost"/>. Renders the session's <see cref="BRenderList"/> straight
/// through the Phase 5 direct-Canvas 2D backend (<see cref="BrowserCanvasRenderer"/>) — the
/// production presentation path — and bridges cursor, clipboard, caret, and system-settings host
/// contracts to the browser.
/// </summary>
    ...
    IUiHost, IUiTextInputHost, IUiClipboardHost, IUiCursorHost, IUiSystemSettingsHost, IDisposable
{
    ...
    /// <summary>Background color cleared behind the render list; the demo updates it on theme changes.</summary>
    ...
    internal BImageHandle CreateImage(BPixelBuffer pixels) => _renderer.CreateImage(pixels);

    internal void ReleaseImage(BImageHandle image)
    {
        if (image.IsValid)
