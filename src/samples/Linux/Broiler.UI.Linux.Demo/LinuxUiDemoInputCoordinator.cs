...
internal sealed class LinuxUiDemoInputCoordinator : IAsyncDisposable
{
    ...
    private readonly InputDeviceId _externalPointerId = InputDeviceId.FromOpaqueValue("linux:demo:x11-pointer");
    ...
    private BSize _viewport = new(900, 600);
    private double _pointerX = 450;
    private double _pointerY = 300;
    ...
    public LinuxUiDemoInputCoordinator(bool enabled, Action<string> log, bool externalPointer = false)
    {
        _enabled = enabled;
    ...
    /// <summary>
    /// Sets the pointer position from the windowing system (X11), keeping the
    /// demo pointer in sync with the real OS cursor instead of accumulating raw
    /// evdev deltas. Coordinates are logical (device-independent) pixels.
    /// </summary>
    ...
    public LinuxUiDemoInputSnapshot Snapshot
    {
        get
        {
            lock (_gate)
            {
                return new LinuxUiDemoInputSnapshot(
                    _enabled,
                    _initialized,
                    _active,
                    _pointerX,
    ...
    public int Drain(UiSession session)
    {
    ...
    public async ValueTask InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        _log("evdev input opened. Events run only while the X11 window is focused; Escape exits the demo loop.");
    }
    ...
    private void LogInputAccessRemediation(LinuxEventDeviceAccessStatus access)
    {
        ...
        {
            _log("  reason: " + access.EventDeviceCount.ToString(System.Globalization.CultureInfo.InvariantCulture) +
    ...
    private void OnMouseMoved(MouseMoveEvent inputEvent)
    {
        ...
        lock (_gate)
        {
            ...
            // When the X11 window drives the pointer, raw evdev deltas are only
            // counted for diagnostics; the real cursor position is authoritative.
            if (_externalPointer)
                return;
...
internal readonly record struct LinuxUiDemoInputSnapshot(
    bool Enabled,
    bool Initialized,
    bool Active,
    double PointerX,
